...
class AdvancedRAG:
    def __init__(self, milvus_uri, cohere_api_key):
        ...
        # Reranker
        self.rerank_compressor = CohereRerank(cohere_api_key=cohere_api_key, top_n=5)
    ...
    def initialize(self, documents):
        ...
        data = [
            {"id": i, "vector": vectors[i], "text": self.texts[i]} for i in range(len(self.texts))
        ]
    ...
    def retrieve(self, query, k=10, use_rerank=True):
        ...
        for rank, idx in enumerate(
            sorted(range(len(bm25_scores)), key=lambda i: bm25_scores[i], reverse=True)[:k]
        ):
            all_scores[idx] = all_scores.get(idx, 0) + 1 / (60 + rank + 1)

        # 向量检索
        query_vector = self.embeddings.embed_query(query)
        results = self.milvus.search(collection_name="documents", data=[query_vector], limit=k)
        ...
        # 3. Rerank
        if use_rerank:
            reranked = self.rerank_compressor.compress_documents(retrieved_chunks, query)
            return reranked
