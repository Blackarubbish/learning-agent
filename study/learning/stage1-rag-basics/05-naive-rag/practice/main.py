...
def ingest_document(session_id: str, file_path: str, filename: str) -> IngestResponse:
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"文档加载失败: {str(e)}") from e
...
async def ingest_file(file: UploadFile = File(...)):  # noqa: B008
    """上传并摄取文档"""
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"摄取失败: {str(e)}") from e
...
def list_sessions():
    """列出已摄取的文档 sessions"""
    sessions = []
    for session_id in session_vectorstores:
