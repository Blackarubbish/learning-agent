...
def ask_question(req: QuestionRequest):
    ...
    return QuestionResponse(
        answer=result["result"], sources=[doc.page_content for doc in result["source_documents"]]
    )
...
retriever = vectorstore.as_retriever(search_kwargs={"k": 3})
qa_chain = RetrievalQA.from_chain_type(
    llm=ChatOpenAI(model="gpt-4o-mini"), retriever=retriever, return_source_documents=True
)
