...
def test_comparison_completeness():
    ...
    for dim_key in required_dimensions:
        ...
        if matched_key is None:
            ...
            for key in result:
                if (
                    any(w in key for w in ["代码", "结构", "组织", "code"])
                    and dim_key == "代码结构"
                ):
                    matched_key = key
                    break
                if (
                    any(w in key for w in ["观测", "调试", "debug", "trace", "observ"])
                    and dim_key == "可观测性"
                ):
                    matched_key = key
                    break
                if any(w in key for w in ["灵活", "flexib", "变更"]) and dim_key == "灵活性":
                    matched_key = key
                    break
                if (
                    any(w in key for w in ["选型", "选择", "推荐", "场景"])
                    and dim_key == "选型建议"
                ):
                    matched_key = key
                    break
