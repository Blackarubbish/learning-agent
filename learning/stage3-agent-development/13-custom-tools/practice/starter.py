"""
自定义工具开发：设计 AI 友好的工具接口

目标：理解"工具工程"的核心原则——工具不只是函数，它是 Agent 的"眼睛和手"。

核心认知（开始前读）：
  - 70% 的工作是工具工程，AI 只完成 30%（AgentGuide 生产实践）
  - 工具返回的信息是 LLM 做下一步决策的唯一依据
  - 信息过载 = 决策瘫痪：10000 行数据对 LLM 来说是噪音
  - 好的错误信息包含：发生了什么 + 为什么 + 下一步可以尝试什么
...
# ═══════════════════════════════════════════
# TODO 1: 信息抽象 — 从"数据倾泻"到"智能摘要"
# ═══════════════════════════════════════════
# 工具返回的信息量直接影响 LLM 的决策质量。
# 太多 → token 过载，LLM 无法聚焦关键信息
# 太少 → LLM 没有足够依据做决策
# 目标：结构化的"刚好够用"
#
# 关键技巧：
...
def naive_search(query: str) -> str:
    ...
    TODO 1.1: 实现一个简单的关键词搜索
    1. 遍历 KNOWLEDGE_BASE，用 query 匹配 title 或 content（不区分大小写）
    2. 返回所有匹配文档的完整标题 + 内容 + 标签
    3. 如果无匹配，返回 "未找到与 '{query}' 相关的文档"
    """
    # TODO: 实现简单搜索
    matches = []
...
def smart_search(query: str, max_results: int = 3) -> str:
    ...
    TODO 1.2: 在 naive_search 基础上升级，实现以下功能：

    1. 相关性分级：标题匹配标"高"，内容匹配标"中"，标题匹配排前面
    2. 结构化输出格式：
       🔍 搜索 'XXX'：共 N 篇匹配，显示前 M 篇

       ## 1. 文档标题 [相关性: 高/中]
          内容预览（最多 150 字，超出加 "..."）
          标签: xxx, xxx

    3. 如果结果数 > max_results，末尾追加引导：
       ---
       还有 X 篇未显示。如需查看更多，可以：
         1. 用更具体的关键词缩小范围
         2. 指定标签筛选
    ...
    for doc in KNOWLEDGE_BASE:
        if query.lower() in doc["title"].lower():
            matches.append({**doc, "relevance": "高"})
        elif query.lower() in doc["content"].lower():
            matches.append({**doc, "relevance": "中"})

    if not matches:
        topics = sorted(set(tag for doc in KNOWLEDGE_BASE for tag in doc["tags"]))
        return (
            f"未找到与 '{query}' 相关的文档。\n"
            f"建议：尝试更宽泛的关键词。可用主题：{', '.join(topics)}"
        )

    # 第二步：按相关性排序，截断到 max_results
    matches.sort(key=lambda d: 0 if d["relevance"] == "高" else 1)
    total = len(matches)
    shown = matches[:max_results]

    # 第三步：格式化输出
    output = f"🔍 搜索 '{query}'：共 {total} 篇匹配，显示前 {min(total, max_results)} 篇\n\n"
    ...
    if total > max_results:
        output += (
            f"---\n"
            f"还有 {total - max_results} 篇未显示。如需查看更多，可以：\n"
            f"  1. 用更具体的关键词缩小范围\n"
            f"  2. 指定标签筛选\n"
        )
...
# ═══════════════════════════════════════════
# TODO 2: 状态反馈 — 让 Agent 知道进展
# ═══════════════════════════════════════════
# 批量操作中，Agent 需要知道：做了多少、成功多少、失败多少、接下来能做什么
# 如果只返回 "处理完成"，Agent 无法判断是否需要重试失败项
...
def batch_process(items_json: str, operation: str) -> str:
    ...
    1. 解析 items_json（JSON 数组字符串），解析失败时返回友好错误（含正确示例）
    2. 验证 operation 是否在支持列表中（summarize / classify / keywords），否则返回可用列表
    3. 遍历 items，逐项处理并记录状态：
       - 以 "err_" 开头的项模拟处理失败
       - 每项输出格式: "  [i/total] ✅/❌ 'item' 操作完成/处理失败"
    4. 末尾输出汇总统计和后续建议：
       📊 批量XX完成: 100% (4/4)
          成功: 3, 失败: 1
       ...（逐项状态）...
       ⚠️ 1 项处理失败。后续建议：(1) 检查失败项 (2) 跳过失败项继续
    """
    items = None
    try:
        items = json.loads(items_json)
    except json.JSONDecodeError:
        return '错误: items_json 不是合法的 JSON 数组字符串。正确示例: \'["文本1", "文本2"]\''

    # TODO: 实现批量处理
    if operation not in ["summarize", "classify", "keywords"]:
        return f"错误: 不支持的操作 '{operation}'。可用操作: summarize / classify / keywords"

    # 判断 items 是否为列表
    if not isinstance(items, list):
        return '错误: items_json 需要是一个 JSON 数组字符串。正确示例: \'["文本1", "文本2"]\''
    ...
    for i, item in enumerate(items, start=1):
        if isinstance(item, str) and item.startswith("err_"):
            details.append(f"  [{i}/{total}] ❌ '{item}' 处理失败")
            failure_count += 1
        else:
            details.append(f"  [{i}/{total}] ✅ '{item}' 操作完成")
            success_count += 1

    completion_rate = math.ceil((success_count + failure_count) / total * 100)
    summary = f"📊 批量{operation}完成: {completion_rate}% ({success_count}/{total})\n"
    summary += f"   成功: {success_count}, 失败: {failure_count}\n"
    summary += "\n".join(details)

    if failure_count > 0:
        summary += f"\n⚠️ {failure_count} 项处理失败。后续建议：(1) 检查失败项 (2) 跳过失败项继续"

    return summary


# ═══════════════════════════════════════════
# TODO 3: 错误恢复接口 — 错误信息即决策依据
# ═══════════════════════════════════════════
# bad:  return "Error: invalid params"
# good: return "❌ weather 端点缺少城市参数。可用城市: ['北京','上海','深圳']。示例: api_fetch('weather', '北京')"
#
# 好的错误信息让 LLM 能自主修正，不需要用户介入
# 注意：用扁平字符串参数而非嵌套 JSON——LLM 更容易正确生成
...
def api_fetch(endpoint: str, param: str = "") -> str:
    ...
    TODO 3: 实现带错误恢复的 API 工具

    1. 验证 endpoint 是否在 MOCK_API 中，不在时列出所有可用端点及用法
    2. 实现三个端点（param 含义因端点而异）：
       - weather: param = 城市名（如 "北京"），返回天气字符串
       - news: param = 类别（如 "tech"/"science"），默认 "tech"，返回新闻列表
       - stock: param = 股票代码（如 "AAPL"），返回价格
    3. 每个端点缺少必填参数时，明确指出缺少什么 + 给出正确示例

    错误信息设计原则：
    - 不要只说"参数错误"，要说哪个参数、为什么错、正确格式是什么
    - 列出可用选项，让 LLM 能自主选择
...
# ═══════════════════════════════════════════
# 5. Agent（复用第 12 章的 ReAct 循环，已提供）
# ═══════════════════════════════════════════
...
if __name__ == "__main__":
    ...
    # TODO: 实现 naive_search 和 smart_search 后取消注释
